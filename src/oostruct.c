...
defclass(at *classname, at *superclass, at *keylist, at *defaults)
{
  ...
  cl = tl_malloc(sizeof(struct class));
  *cl = object_class;
...
new_oostruct(at *cl)
{
  ...
  len = c->slotssofar;
  s = tl_malloc(sizeof(struct oostruct)+(len-1)*sizeof(struct oostructitem));
  s->size = len;
  s->class = cl;
  LOCK(cl);
...
update_hashtable(class *cl)
{
 ...
 restart:
  ...
  ifn (cl->hashtable = tl_malloc( size * sizeof(struct hashelem) ))
