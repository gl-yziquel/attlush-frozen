...
static int matrix_sweep_and_exec (int ndim_still, int *pdim,
				  int *pmod1, gptr pdata1,
				  int *pmod2, gptr pdata2,
...
/* LUfact carries out LU factorization of aits argument Lush matrix
...
/* mseigvv
   IN  : a  : pointer on a Lush symetric matrix
         n  : matrix size
	 mi : "modulo" on lines
	 mj : "modulo" on columns
...
#define LUSH_INITIAL_IDENTITY_SIZE 32
...
#define LUSH_NSTACK 	80
#define LUSH_M 		7
#define LUSH_SWAPF(x,y) 	{ float tmp=(x); (x)=(y); (y)=tmp; }
#define LUSH_SWAPI(x,y) 	{ int tmp=(x); (x)=(y); (y)=tmp; }
...
qsort_mod_idx (float *arr, int *idx, int n, int mod, int modi)
{
  ...
  int jstack = 0;
  int istack[LUSH_NSTACK];
  float a;

  m = n - 1;
  l = 0;
  for (;;) {                    /* Sort segment [l,m] */
    if (m - l < LUSH_M) {            /* Insertion sort */
      for (j = l + 1; j <= m; j++) {
        a = arr[j * mod];
        p = idx[j * modi];
        for (i = j - 1; i >= l; i--) {
      ...
      k = (l + m) >> 1;
      LUSH_SWAPF (arr[k * mod], arr[(l + 1) * mod]);
      LUSH_SWAPI (idx[k * modi], idx[(l + 1) * modi]);
      if (arr[(l + 1) * mod] > arr[m * mod]) {
        LUSH_SWAPF (arr[(l + 1) * mod], arr[m * mod]);
        LUSH_SWAPI (idx[(l + 1) * modi], idx[m * modi]);
      }
      if (arr[l * mod] > arr[m * mod]) {
        LUSH_SWAPF (arr[l * mod], arr[m * mod]);
        LUSH_SWAPI (idx[l * modi], idx[m * modi]);
      }
      if (arr[(l + 1) * mod] > arr[l * mod]) {
        LUSH_SWAPF (arr[(l + 1) * mod], arr[l * mod]);
        LUSH_SWAPI (idx[(l + 1) * modi], idx[l * modi]);
      }
      ...
      for (;;) {
        ...
        if (j < i)
          break;
        LUSH_SWAPF (arr[i * mod], arr[j * mod]);
        LUSH_SWAPI (idx[i * modi], idx[j * modi]);
      }
...
mseek (at *m0, flt x, int way, at *midx)
{
  ...
  if (N == 0) {
    N = LUSH_INITIAL_IDENTITY_SIZE;
    identity = (int *) tl_malloc ((sizeof (int)) * LUSH_INITIAL_IDENTITY_SIZE);
...
matrix_sweep_and_exec (int ndim_still, int *pdim,
		       int *pmod1, gptr pdata1, /* pdata1 null => unused */ 
		       int *pmod2, gptr pdata2, /* pdata2 null => unused */ 
		       flt x, flt y, int *pcount,
                       void (*op) (flt *, flt *, flt, flt, int *))
{
  int i;
