	      ...
	      unsigned int sx, unsigned int sy)
{
  ...
  if (!image) {
    if (sx * sy > 9 && xdef.depth >= MINDEPTH) /* test if ok */
      return FALSE;
    else
      return TRUE;
  }
  if (sx == 1 && sy == 1) {
    ...
    ifn (sx>0 && sy>0)
      return FALSE;
    ifn (image2 = tl_malloc(sx_w * sy_h * sizeof(int)))
      return FALSE;
...
void
x11_get_image(struct window *linfo, unsigned int *image, int x, int y, unsigned int w, unsigned int h)
{
  register int i,j;
  ...
  XImage *ximage;
  ximage = XGetImage(xdef.dpy, info->backwin, x, y, w, h, 0xffff, ZPixmap);
  im = image;
  for (i=0; i<h; i++) {
    for (j=0; j<w; j++) {
...
int 
x11_get_mask(struct window *linfo, unsigned int *red_mask, unsigned int *green_mask, unsigned int *blue_mask)
{
