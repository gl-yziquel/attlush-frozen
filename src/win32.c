...
/************************************************************************
   WinT-Lisp3:   (C) LYB 1997
   win32.cpp:  OS interface file for WIN32
   $Id: win32.c,v 0.1 2001/10/31 17:31:10 profshadoko Exp $
*********************************************************************** */
...
/* lushport -- rendez-vous point for passing messages between Lush and WINLush */

LushPORT *lushport = NULL;
...
/* lushsend -- send a message to Lush environment */

void 
lushsend(LushMSG msgcode, LUSHARG *msgarg)
{
    DWORD status;
    if (!lushport)
      abort("Internal error: Lush environment not found");
    lushport->msgcode = msgcode;
    lushport->msgarg = msgarg;
    SetEvent(lushport->hevSendMessage);
  again:
    {
      int i;
      MSG msg;
	...
	DispatchMessage(&msg);
      ...
      status = WaitForSingleObject(lushport->hevReplyMessage, 0);
	...
	if ((*waitproc[i])(NULL))
      ...
      status = MsgWaitForMultipleObjects(1, 
		  &lushport->hevReplyMessage, 
		  FALSE, INFINITE, QS_ALLINPUT );
      if (status==WAIT_OBJECT_0)
	return;
      if (status==WAIT_OBJECT_0 + 1)
	goto again;
      abort("Internal error: waiting winlush answer");
    }
}
...
win32_exit(int retcode)
{
	...
	lushsend(LUSH_EXIT, (LUSHARG*)&arg_simple);
...
win32_popen(char *fname, char *mode)
{
  ...
  else
  {
    ...
    if (_osver & 0x8000)
      error(NIL,"Windows95 bug(?) forbids using pipes from WinLush",NIL);
#endif
    /* Create WIN32 pipe */
    ...
    if (! pIob)
      goto err;
    /* Create process */
    hConsole = (lushport ? lushport->hfStdoutInheritable : NULL);
    if (bReadP)
...
/* 
 * (winlushp)
 * Returns TRUE if running WINLush instead of LushCMD
 */
...
DX(xwinedit)
{
  ...
  if (!lushport)
    error(NIL,"Not running in WinLush",NIL);
  arg_edit.string = s;
  arg_edit.status = mode;
  lushsend(LUSH_WINEDIT, (LUSHARG*)&arg_edit);
...
DX(xwinsys)
{
  ...
  else if (arg==at_true) 
  {
    hIn = NULL;
    if (lushport)
      error(NIL,"WinLush has no valid standard input handle",NIL);
  } 
  ...
  else if (arg==at_true) 
  {
    hOut = NULL;
    if (lushport)
      hOut = lushport->hfStdoutInheritable;
  } 
  else if (EXTERNP(arg, &symbol_class))
  {
    outpipesym = arg;
  ...
  else if (arg==at_true) 
  {
    hErr = NULL;
    if (lushport)
      hErr = lushport->hfStdoutInheritable;
  } 
 ...
 * Control display of main winlush window
 */
...
DX(xwin_show_workbench)
{
  ...
  if (lushport)
  {
    ...
    lushsend(LUSH_SHOWWORKBENCH, (LUSHARG*)&arg);
    return TLtrue();
  }
  return NIL;
}
...
/* Exception filter called by WinLush */
int 
lushexception(LPEXCEPTION_POINTERS ExceptionInfo)
{
...
/* lushevent_add, lushevent_test -- normally implemented in win_driver.c */

#ifdef NOGRAPHICS

BOOL lushevent_test(void) 
{ return FALSE; }

BOOL lushevent_add(void *wid, int code, int arg, int x, int y)
{ return TRUE; }
...
wait_trigger(void)
{
  if (lushport)
  {
    struct msg_simple arg_simple;
    arg_simple.status = 0;
    lushsend(LUSH_WAITEVENT, (LUSHARG*)&arg_simple);
  }
  else
  {
    /* do not hog computer */ 
...
console_getline(char *prompt, char *buf, int size)
{
  if (lushport)
  {
      ...
      for (;;)
      {
	lushsend(LUSH_GETLINE, (LUSHARG*)&arg_getline);
